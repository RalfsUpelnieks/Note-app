...
function Header({isAdmin, pages}: any) {
    ...
    return (
        <header className='flex fixed left-0 right-0 top-0 bottom-auto px-4 justify-between z-10 items-center bg-[rgb(233,233,233)] shadow-[0px_3px_5px_-1px_rgba(0,0,0,0.2),_0px_1px_14px_0px_rgba(0,0,0,0.12)]'>
            ...
            {!isAdmin ? 
                <div className=' relative w-2/6 mx-3 lg:ml-[12%]'>
                    <input className='peer' type="text" placeholder="Search..." onChange={handleChange}/>
                    <div className='hidden peer-focus:flex peer-focus-within:flex hover:flex flex-col w-[calc(100%-2px)] top-8 absolute bg-white rounded-b-[4px] border border-solid border-t-neutral-200 border-[#ced4da]'>
                    {pageList.length !== 0 ? (
                        pageList.map((object) => {
                            return (
                                <Link to={`/page/${object.pageId}`} className="">
